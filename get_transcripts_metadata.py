    ...
    with open(os.path.join(script_directory, "gulf_channels_processed.txt"), "a") as processed_file:
        ...
        for i, playlist_url in enumerate(playlist_urls):
            ...
            try:
              ...
              lev_channel_df = pd.DataFrame(results, columns=["video_url", "duration", "upload_date"])
              lev_channel_df["playlist_url"] = playlist_url
              lev_channel_df["duration_in_sec"] = lev_channel_df["duration"].apply(lambda x: timeparse(x))
